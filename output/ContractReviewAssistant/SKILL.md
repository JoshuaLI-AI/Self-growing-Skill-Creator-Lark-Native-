---
name: ContractReviewAssistant
version: 1.0.0
description: "合同审核AI助手。当用户需要审核合同、检查合规性、或查询合同审核流程规范时使用。"
---
...
LARK_KNOWLEDGE_BASE:
  bitable_app_token: "XcO5bo9FhaTzaasYCHVcYbqznwf"              # Bitable App Token（格式 BzQ...）
  wiki_space_id: "7625923522954808540"                    # Wiki 空间 ID（格式 Spacexxx）
  tables:
    knowledge_map: "tblyzOe8FNjXxnq7"               # KM 表 table_id（格式 tblxxx）
    experience_log: "tblyGZugaussCjM0"              # EL 表 table_id（格式 tblxxx）
  wiki_nodes:
    reference: "VLw0wOHvfiIXVfkzrrxcaVClnTh"                     # 参考文件节点 node_token
    archives: "Sq6Ew6RytibDPnk4rcPcCp5Vn3g"                      # 工作归档节点 node_token
...
# 从需求访谈中收集的领域变量列表：
  - name: "NegotiationRole"
    type: enum
    options: [甲方, 乙方, 中立方]
    hint: "从用户语言识别我方在合同中的角色；无法识别时留空"
    el_field: "NegotiationRole"
...
```bash
lark-cli base +record-list --base-token XcO5bo9FhaTzaasYCHVcYbqznwf --table-id tblyGZugaussCjM0
```
...
```bash
lark-cli base +record-list --base-token XcO5bo9FhaTzaasYCHVcYbqznwf --table-id tblyzOe8FNjXxnq7
```
...
REF_OBJ=$(lark-cli wiki spaces get_node \
  --params '{"token":"VLw0wOHvfiIXVfkzrrxcaVClnTh"}' \
